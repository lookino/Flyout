...
2. In the macro body, start by typing `/flyout` and then the names of the spells separated by a semicolon
   - To use a specific rank, you can write `Shadow Bolt(Rank 1)` (omitting the rank will use the highest rank)
...
The add-on relies on functions provided by the default action bar and action button logic implemented by Blizzard. Some add-ons, such as action bars replacement addons and/or macro add-ons override these functions or use their own and therefore may not work together with this add-on.
